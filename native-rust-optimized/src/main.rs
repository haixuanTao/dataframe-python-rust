...
use utils::{NativeDataFrame, NativeDataFrameRaw};
...
fn use_native_rust(
    ...
    let hash_wikipedia: HashMap<String, utils::WikiDataFrame> = rdr_wiki
        .deserialize()
        .into_iter()
        .filter_map(|result: csv::Result<utils::WikiDataFrame>| {
            result.map(|x| (x.language.clone(), x)).ok()
        })
        .collect();

    let file = File::open(path)?;
    let mut rdr = csv::ReaderBuilder::new().delimiter(b',').from_reader(file);
    let groups_hash = rdr.deserialize().into_iter().fold(
        HashMap::<String, (utils::GroupBy, usize)>::new(),
        |mut hash_group, record: csv::Result<utils::NativeDataFrame>| {
            ...
            group.owner_undeleted_answer_count_at_post_time +=
                record.owner_undeleted_answer_count_at_post_time;
    ...
    let groups = groups_hash.into_iter().map(|(_, (mut group, count))| {
